...
class ProductPage extends Component {
  ...
  handleClick = (id) => {
    ...
    if (quantityWanted <= 0 || quantityWanted === null) {
      return this.setState({ message: 'Please Choose a valid quantity' })
    }

    if (quantityWanted > this.props.product.quantity) {
      return this.setState({ message: `This product is out of stock (
        current stock : ${this.props.product.quantity})` })
    }
